...
const LFO_KNOB_IDS = { 
    101: {lfo: 0, param: 'wave'}, 103: {lfo: 1, param: 'depth'}, 104: {lfo: 2, param: 'depth'}, 
    105: {lfo: 1, param: 'wave'}, 106: {lfo: 0, param: 'depth'}, 107: {lfo: 3, param: 'dest'}, 
    108: {lfo: 0, param: 'rate'}, 109: {lfo: 1, param: 'rate'}, 110: {lfo: 2, param: 'rate'}, 
    111: {lfo: 3, param: 'rate'}, 112: {lfo: 2, param: 'wave'}, 113: {lfo: 3, param: 'wave'}, 
    100: {lfo: 3, param: 'depth'}, 102: {lfo: 2, param: 'dest'}, 114: {lfo: 0, param: 'dest'}, 
...
process(i, o, p) {
        ...
        // --- MAIN AUDIO LOOP (Runs 128 times per block) ---
        for (let i = 0; i < oL.length; i++) {

            // 1. --- CALCULATE LFOs (Per Sample) ---
            ...
            for (let l = 0; l < 4; l++) {
                ...
                switch (lfo.wave) {
                    ...
                    case 2: val = lfo.phase < Math.PI ? 1 : -1; break; // Perfect Square
            ...
            // 2. --- LFO CROSS-MODULATION ---
            // (Simplified for speed: Modulate rates/depths based on destination chain)
            // For this test, we simply calculate the final parameter modifications
            let modulatedFx = {};
            for (let l = 0; l < 4; l++) {
                const lfo = this.lfoParams[l];
                // Check destinations (simple version)
                const destinations = (lfo.destChain && lfo.destChain.length) ? lfo.destChain : 
                                     (lfo.dest !== -1 ? [lfo.dest] : []);
                                     
                if (destinations.length) {
                    for (const dest of destinations) {
                        if (!modulatedFx[dest]) modulatedFx[dest] = 0;
                        modulatedFx[dest] = Math.max(-1, Math.min(1, modulatedFx[dest] + rawLfoOutputs[l]));
                    }
                }
            }

            // 3. --- APPLY MODULATION TO PARAMS ---
            ...
            // 6. --- FILTERS (Per Sample Updates) ---
            this.updateFilterCoefficients(this.filterOsc1Coeffs, currentParams[20], currentParams[28]);
            let s1_f=0; 
            if (this.envStage1 !== 'off'){ 
                const c1=this.filterOsc1Coeffs; 
                s1_f=c1.b0*s1_e+c1.b1*this.filter_osc1_x1+c1.b2*this.filter_osc1_x2-c1.a1*this.filter_osc1_y1-c1.a2*this.filter_osc1_y2; 
                this.filter_osc1_x2=this.filter_osc1_x1;this.filter_osc1_x1=s1_e;this.filter_osc1_y2=this.filter_osc1_y1;this.filter_osc1_y1=s1_f; 
            } 
            
            this.updateFilterCoefficients(this.filterOsc2Coeffs, currentParams[21], currentParams[29]);
            let s2_f=0; 
            if (this.envStage2 !== 'off'){ 
                const c2=this.filterOsc2Coeffs; 
                s2_f=c2.b0*s2_e+c2.b1*this.filter_osc2_x1+c2.b2*this.filter_osc2_x2-c2.a1*this.filter_osc2_y1-c2.a2*this.filter_osc2_y2; 
                this.filter_osc2_x2=this.filter_osc2_x1;this.filter_osc2_x1=s2_e;this.filter_osc2_y2=this.filter_osc2_y1;this.filter_osc2_y1=s2_f; 
            }
            ...
            if (dV>0.01){ 
                const dr=1+dV*19; const k=2*dr/(1+dr); 
                s_L=(1+k)*s_L/(1+k*Math.abs(s_L)); s_R=(1+k)*s_R/(1+k*Math.abs(s_R)); 
                const nS=Math.max(2,Math.floor(Math.pow(1-dV,2.5)*64)); const sS=2.0/nS; 
            ...
            if(rW>0){ 
                let cO_L=0,cO_R=0; 
                this.combsL.forEach(c=>cO_L+=c.process(s_L*0.1)); 
                this.combsR.forEach(c=>cO_R+=c.process(s_R*0.1)); 
                let aO_L=this.allpassesL[1].process(this.allpassesL[0].process(cO_L)); 
                let aO_R=this.allpassesR[1].process(this.allpassesR[0].process(cO_R)); 
                s_L=(s_L*(1-rW))+(aO_L*rW); s_R=(s_R*(1-rW))+(aO_R*rW); 
            }
            ...
            // Recording
            if(this.isRecording){ 
                this.recL[this.recIndex]=outL; this.recR[this.recIndex]=outR; 
                this.recIndex++; 
