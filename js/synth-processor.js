...
class SynthProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.lfoParams = [ { rate: 0, depth: 0, wave: 0, dest: 0, phase: 0, lastRandom: 0 }, { rate: 0, depth: 0, wave: 0, dest: 0, phase: 0, lastRandom: 0 }, { rate: 0, depth: 0, wave: 0, dest: 0, phase: 0, lastRandom: 0 }, { rate: 0, depth: 0, wave: 0, dest: 0, phase: 0, lastRandom: 0 }, ];
        this.lfoOutputs = [0,0,0,0];
        this.port.onmessage = ({ data: { type, data } }) => {
            const { voice, freq, id, value, lfoId, param } = data || {};
            switch (type) {
                case 'noteOn': if (voice === 0) { this.noteOn1 = true; this.targetFrequency1 = freq; if (this.params[0] < 0.01) this.currentFrequency1 = freq; this.envStage1 = 'attack'; } else { this.noteOn2 = true; this.targetFrequency2 = freq; if (this.params[0] < 0.01) this.currentFrequency2 = freq; this.envStage2 = 'attack'; } break;
                case 'noteOff': if (voice === 0) { this.envStage1 = 'release'; } else { this.envStage2 = 'release'; } break;
                ...
                case 'setFx':
                    ...
                    break;
                case 'setLfo': if (lfoId >= 0 && lfoId < this.lfoParams.length && this.lfoParams[lfoId][param] !== undefined) { this.lfoParams[lfoId][param] = value; } break;
                case 'requestLfoUpdate': this.port.postMessage({type:'lfoUpdate', data: this.lfoOutputs}); break;
    ...
    process(i,o,p){
        ...
        for (let i = 0; i < 4; i++) {
            const sourceLfo = this.lfoParams[i];
            if (sourceLfo.dest !== 0 && LFO_KNOB_IDS[sourceLfo.dest]) {
                let initialOutput = 0;
                switch (sourceLfo.wave) {
                    case 0: initialOutput = Math.sin(sourceLfo.phase); break;
                    case 1: initialOutput = Math.asin(Math.sin(sourceLfo.phase)) * (2 / Math.PI); break;
                    case 2: initialOutput = sourceLfo.phase < Math.PI ? 1 : -1; break;
                    case 3: initialOutput = (sourceLfo.phase / Math.PI) - 1; break;
                    case 4: initialOutput = 1 - (sourceLfo.phase / Math.PI); break;
                    case 5: initialOutput = sourceLfo.lastRandom; break;
                }
                const modAmount = initialOutput * sourceLfo.depth;
                const targetInfo = LFO_KNOB_IDS[sourceLfo.dest];
                modulatedParams[targetInfo.lfo][targetInfo.param] = Math.max(0, Math.min(1, modulatedParams[targetInfo.lfo][targetInfo.param] + modAmount));
            }
        }

        let finalLfoOutputs = [0, 0, 0, 0];
        for (let i = 0; i < 4; i++) {
            const mParams = modulatedParams[i];
