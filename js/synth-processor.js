...
class SynthProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.params=new Array(64).fill(0.0);
        // Envelope times
        ...
        this.port.onmessage = ({ data: { type, data } }) => {
            const { voice, freq, id, value, lfoId, param, bpm, bufferL, bufferR, grid } = data || {};
            switch (type) {
                case 'noteOn':
                    if (voice === 0) { 
                        this.noteOn1 = true; this.targetFrequency1 = freq; 
                        if (this.params[0] < 0.01) this.currentFrequency1 = freq; 
                        this.envStage1 = 'attack'; 
                    } else { 
                        this.noteOn2 = true; this.targetFrequency2 = freq; 
                        if (this.params[0] < 0.01) this.currentFrequency2 = freq; 
                        this.envStage2 = 'attack'; 
                    } 
                    break;
                case 'noteOff': 
                    if (voice === 0) this.envStage1 = 'release'; else this.envStage2 = 'release'; 
                    break;
                case 'setFreq': 
                    if (voice === 0) this.targetFrequency1 = freq; else this.targetFrequency2 = freq; 
                    break;
                
                // --- DRUM MESSAGES ---
                case 'setBPM': this.bpm = Math.max(20, Math.min(300, bpm)); break;
                case 'loadDrumSample': this.drumSampleL = bufferL; this.drumSampleR = bufferR; this.drumPhase = 0; break;
                case 'setDrumMode': this.isDrumMode = !!value; break;
                case 'setDrumGrid': if (grid && grid.length === 16) this.drumGrid = new Uint8Array(grid); break;

                case 'setFx':
                    if (id >= 0 && id < this.params.length) { this.params[id] = value; }
                    if(id===2){ this.updateFilterCoefficients(this.filterCoeffs, value, 0.0); }
                    else if(id===8){ this.attackTime=0.001+Math.pow(value,2)*2; } 
                    else if(id===9){ this.decayTime=0.001+Math.pow(value,2)*2; }
                    else if(id===10){ this.sustainLevel=value; } 
                    else if(id===11){ this.releaseTime=0.001+Math.pow(value,2)*1.25; this.releaseRate=Math.exp(-1/(this.releaseTime*sampleRate)); }
                    // Reverb Params Mapping (User Custom Settings)
                    else if(id===13){ 
                ...
                case 'requestLfoUpdate':
                    ...
                    break;
                case 'startRecording': 
                    { const bs=(data&&data.blockSize|0)||8192; this.recordBlockSize=Math.max(128,bs); this.recL=new Float32Array(this.recordBlockSize); this.recR=new Float32Array(this.recordBlockSize); this.recIndex=0; this.isRecording=true; break; }
                case 'stopRecording': 
                    { if(this.isRecording){if(this.recIndex>0){const i=new Float32Array(this.recIndex*2); for(let j=0,k=0;j<this.recIndex;j++){i[k++]=this.recL[j];i[k++]=this.recR[j];} this.port.postMessage({type:'audio',data:i},[i.buffer]); this.recIndex=0;} this.isRecording=false; this.port.postMessage({type:'recordingStopped'});} break;}
            }
        };
    ...
    process(i,o,p){
        ...
        for (let l = 0; l < 4; l++) {
            ...
            if (lfo.depth > 0.001) {
                ...
                if (lfo.wave === 5 && oldPhase > lfo.phase) lfo.lastRandom = Math.random() * 2 - 1;
            }
        }

        // LFO Feedback & Cross Modulation Logic
        ...
        for (let l = 0; l < 4; l++) {
            ...
            if (destinations.length && rawLfoOutputs[l] !== 0) {
                for (const dest of destinations) {
                    ...
                    const loopKey = `${l}->${targetIndex}:${targetLfoInfo.param}`;
                    if (appliedLfoTargets.has(loopKey)) continue;
                    appliedLfoTargets.add(loopKey);
                    
                    const targetLfo = this.lfoParams[targetIndex];
                    if (targetLfoInfo.param === 'rate') {
                       const modRate = Math.max(0, Math.min(1, targetLfo.rate + rawLfoOutputs[l]));
                       const rateHz = MIN_LFO_RATE_HZ * Math.pow(LFO_RATE_RANGE_RATIO, modRate);
                       const phaseInc = (2 * Math.PI * rateHz * blockSize) / sr;
        ...
        // --- DRUM SLICE MODULATION ---
        const sliceModulation = modulatedFx[FX_PARAM_SLICE_MOD] || 0;
        ...
        const getWaveSample = (phase, waveType) => {
            switch (waveType) {
                case 0: return (phase / Math.PI) - 1.0; 
                case 1: return phase < Math.PI ? 1.0 : -1.0; 
                case 2: return Math.sin(phase); 
                case 3: return (2 / Math.PI) * Math.asin(Math.sin(phase)); 
        ...
        // --- DRUM RATE CALCULATION ---
        let drumIncrement = 0;
        if (this.drumSampleL && this.isDrumMode) {
            const totalSteps = 16;
            const samplesPerStep = this.drumSampleL.length / totalSteps;
            const secondsPer16th = 60 / (this.bpm * 4);
            const targetSamplesPer16th = secondsPer16th * sr;
            drumIncrement = samplesPerStep / targetSamplesPer16th;
        }
        ...
        for(let i=0;i<blockSize;i++){
            ...
            if(this.noteOn1 || this.envStage1 === 'release'){
                const o1_1=getWaveSample(this.phase1_1, waveType1); this.phase1_1=(this.phase1_1+2*Math.PI*this.currentFrequency1/sr)%(2*Math.PI);
                const o2_1=getWaveSample(this.phase2_1, waveType1); this.phase2_1=(this.phase2_1+2*Math.PI*this.currentFrequency1*dA1/sr)%(2*Math.PI);
                const o3_1=getWaveSample(this.phase3_1, waveType1); this.phase3_1=(this.phase3_1+2*Math.PI*(this.currentFrequency1/2)/sr)%(2*Math.PI);
                s1=(o1_1+o2_1)*0.5; s1 = (s1 + (o3_1 * currentParams[3])) * 0.8;
            }
            if(this.noteOn2 || this.envStage2 === 'release'){
                const o1_2=getWaveSample(this.phase1_2, waveType2); this.phase1_2=(this.phase1_2+2*Math.PI*this.currentFrequency2/sr)%(2*Math.PI);
                const o2_2=getWaveSample(this.phase2_2, waveType2); this.phase2_2=(this.phase2_2+2*Math.PI*this.currentFrequency2*dA2/sr)%(2*Math.PI);
                const o3_2=getWaveSample(this.phase3_2, waveType2); this.phase3_2=(this.phase3_2+2*Math.PI*(this.currentFrequency2/2)/sr)%(2*Math.PI);
                s2=(o1_2+o2_2)*0.5; s2 = (s2 + (o3_2 * currentParams[3])) * 0.8;
            }

            s1 += (Math.random() - 0.5) * 0.00001; s2 += (Math.random() - 0.5) * 0.00001;
            ...
            this.smoothedRes1 += (currentParams[28] - this.smoothedRes1) * 0.05;
            ...
            this.smoothedRes2 += (currentParams[29] - this.smoothedRes2) * 0.05;
            ...
            // --- DRUM ENGINE MIX ---
            if (this.isDrumMode && this.drumSampleL) {
                const sampleLen = this.drumSampleL.length;
                const stepLen = sampleLen / 16;
                const currentLinearStep = Math.floor(this.drumPhase / stepLen);
                
                // LFO Slice Offset
                let sliceOffsetSteps = 0;
                if (Math.abs(sliceModulation) > 0.01) {
                    sliceOffsetSteps = Math.floor(sliceModulation * 16); 
                }
                const effectiveStep = (currentLinearStep + sliceOffsetSteps + 16) % 16;
                const gateOpen = this.drumGrid[currentLinearStep % 16] === 1;
                
                if (gateOpen) {
                    const offsetInStep = this.drumPhase % stepLen;
                    const readIdx = Math.floor((effectiveStep * stepLen) + offsetInStep) % sampleLen;
                    const dL = this.drumSampleL[readIdx];
                    const dR = this.drumSampleR ? this.drumSampleR[readIdx] : dL;
                    s_L += dL * 0.8;
                    s_R += dR * 0.8;
                }
                this.drumPhase += drumIncrement;
                if (this.drumPhase >= sampleLen) this.drumPhase -= sampleLen;
            }

            // --- FX CHAIN ---
            this.smoothedDist += (currentParams[1] - this.smoothedDist) * 0.0025;
            const dV=this.smoothedDist; const dryL=s_L; const dryR=s_R;
            const targetDistWet = Math.max(0, Math.min(1, (dV - 0.001) * 5));
            this.distWet += (targetDistWet - this.distWet) * 0.01;
            let distL = s_L; let distR = s_R;
            if (dV > 0.001) {
                const distDrive = dV <= 0.5 ? 0.5 * Math.pow(dV / 0.5, 2) : dV;
                const dr = 1 + distDrive * 19; const k = 2 * dr / (1 + dr);
                distL = (1 + k) * distL / (1 + k * Math.abs(distL));
                distR = (1 + k) * distR / (1 + k * Math.abs(distR));
                const nS = Math.max(2, Math.floor(Math.pow(1 - distDrive, 2.5) * 128)); const sS = 2.0 / nS;
                ...
                if (dV < 0.5) {
                    ...
                    this.distLpL += alpha * (distL - this.distLpL); this.distLpR += alpha * (distR - this.distLpR);
                    distL = this.distLpL; distR = this.distLpR;
                } else { this.distLpL = distL; this.distLpR = distR; }
            } else { this.distLpL = distL; this.distLpR = distR; }
            const wetMix = Math.min(1, Math.max(0, this.distWet));
            s_L = dryL + (distL - dryL) * wetMix; s_R = dryR + (distR - dryR) * wetMix;

            if(currentParams[5] > 0){ 
                const tremRateHz = 2 + (Math.pow(currentParams[5], 3) * 500);
                const tD = currentParams[5] * 0.8; const t = Math.sin(this.tremoloPhase) * tD + (1.0 - tD); 
                s_L *= t; s_R *= t; 
            ...
            } else { this.delayBufferL[this.delayWritePos] = 0; this.delayBufferR[this.delayWritePos] = 0; }
            this.delayWritePos = (this.delayWritePos + 1) % this.delayBufferL.length;

            this.zitaPreL[i] = s_L; this.zitaPreR[i] = s_R;
        }

        // --- BLOCK PROCESSING: REVERB ---
        ...
        for(let i=0; i<blockSize; i++) {
            ...
            if(rW > 0) {
                const wetL = this.zitaWetL[i]; const wetR = this.zitaWetR[i];
                s_L = (s_L * (1 - rW)) + (wetL * rW); s_R = (s_R * (1 - rW)) + (wetR * rW);
            }
            ...
            let finalL = yL * currentParams[7]; let finalR = _yR * currentParams[7];
            finalL = Math.tanh(finalL * 1.2); finalR = Math.tanh(finalR * 1.2);

            oL[i] = finalL; oR[i] = finalR;
            if(this.isRecording){ 
                this.recL[this.recIndex] = finalL; this.recR[this.recIndex] = finalR; this.recIndex++; 
                if(this.recIndex >= this.recordBlockSize){ 
                    const il=new Float32Array(this.recIndex*2); 
                    for(let j=0,k=0;j<this.recIndex;j++){il[k++]=this.recL[j];il[k++]=this.recR[j];} 
                    this.port.postMessage({type:'audio',data:il},[il.buffer]); this.recIndex=0; 
                }
            }
        }
